...
function coupon_options(){
	...
	$sections[] = array(
		...
		'fields' => array(
			...
			array(
				...
				'title' => __('Display Daily Deals', 'coupon') ,
	...
	**********************************************************************/
	...
	$sections[] = array(
		...
		'title' => __('Meals Campaign', 'coupon') ,
		...
		'desc' => __('This is weekly campaign.', 'coupon'),
		...
		'fields' => array(
			...
			array(
				...
				'title' => __('Total Meals Served', 'coupon') ,
			...
			array(
				...
				'title' => __('This weeks Goal', 'coupon') ,
			...
			array(
				...
				'title' => __('Meals Served this week', 'coupon') ,
...
function coupon_widgets_init(){
	...
	register_sidebar(array(
			...
			'name' => __('Cause of the week', 'coupon') ,
		...
		register_sidebar(array(
			...
			'name' => __('Show Hope helps', 'coupon') ,
		...
		register_sidebar(array(
			...
			'description' => __('Appears after the banner of the home page.', 'coupon')
		...
		register_sidebar(array(
			...
			'name' => __('Contact Page Right Top', 'coupon') ,



			'id' => 'contact_page_right_top',
			...
			'before_title' => '<div class="caption category-caption"><h2>',



			'after_title' => '</h2><span class="sep"></span> </div>',



			'description' => __('Appears at the right top of the contact page.', 'coupon')
		...
		register_sidebar(array(
			...
			'name' => __('Contact Page Right bottom', 'coupon') ,



			'id' => 'contact_page_right_bottom',
			...
			'description' => __('Appears at the right bottom of the contact page.', 'coupon')
...
/*?>add_action( 'show_user_profile', 'couponer_edit_user_pic' );



add_action( 'edit_user_profile', 'couponer_edit_user_pic' );
...
function couponer_edit_user_pic( $user ){
    ...
    ?>
        ...
        <table class="form-table">
            ...
            <tr>
                ...
                <th><label for="avatar"><?php _e( 'Profile Picture', 'coupon' ); ?></label></th>
				...
				<td style="width: 50px;" valign="top">
					...
					<?php echo get_author_imm( '',$user->ID ); ?> 
                ...
                <td>
                	...
                	<input type="file" id="imgInp" class="upload" name="avatarEEE" />
...
add_action( 'personal_options_update', 'couponer_save_user_pic_meta' );



add_action( 'edit_user_profile_update', 'couponer_save_user_pic_meta' );
...
function couponer_save_user_pic_meta( $user_id ){
	...
	if( isset( $_FILES['avatarEEE']['name'] ) ){
...
function get_author_imm( $avatard = '', $id_or_email, $size = 96, $default = '', $alt = false ) {
	...
	$user_mffeta = get_user_meta( $id_or_email, 'coupon_user_meta' );



	$user_mffeta = array_shift( $user_mffeta );
	...
	if(empty($user_mffeta['avatar'])){



		$user_mffeta['avatar'] = 'http://0.gravatar.com/avatar/3cfdcb5ae66e0ea6b7b38b04df78de4a?s=96&d=mm';
	...
	$avatard       = "<img src='" . $user_mffeta['avatar'] . "' class='avatar avatar-{$size} photo' height='{$size}' width='{$size}' />";
...
function coupon_setup(){
	...
	if (function_exists('add_image_size')){
		...
		add_image_size( 'shop_logo_icon', 0, 150, false );
...
function coupon_calculate_ratings( $id){
	...
	$result = coupon_get_rate_average( $id );
	...
	if( $result->count > 0 ) {
		...
		$average = get_post_meta( $id, 'coupon_average_rate', true );
	...
	if(is_front_page()){
		...
		return '<div class="pull-left">'.$rate.'</div><div class="pull-right"><span> ('.round($average).' Stars)</span></div>';

	}else{

		return '<div class="pull-left">'.$rate.'</div><div class="pull-right"><span> ('.round($average).')</span></div>';
...
function coupon_write_ratings(){
	...
	//echo $post_id.' - '.'coupon_average_rate-'.$rate.' - '.$average;
...
if( !function_exists('coupon_scripts_styles') ){
	...
	function coupon_scripts_styles(){
		...
		wp_enqueue_script( 'coupon-jquery-countdown',  get_template_directory_uri() . '/js/jquery.countdown.min.js', false, false, true );
...
if( !function_exists( 'coupon_code_type' ) ){
	...
	function coupon_code_type( $code_type ){
		...
		switch( $code_type ){
			...
			case '1' : return '<div class="featured-tag"><p>'.__( 'FEATURED', 'coupon' ).'</p></div>'; break;
...
function allMealsofShop($shop_id){
...
$main_query = new WP_Query($args);
			...
			if( $main_query->have_posts() ){
				...
				while( $main_query->have_posts() ){
					...
					$coupon_meals = $coupon_meals + coupon_get_smeta( 'code_meals', $code_meta, '' );
...
function coupon_custom_meta_boxes(){
	...
	$shops_meta = array(
		...
		, array(
			...
			'name' => __( 'Shop Short Description', 'coupon' ),
			...
			'name' => __( 'Average Savings ($)', 'coupon' ),
	...
	function coupon_home_list( $codes ){
		...
		if( !empty( $codes ) ){	
			...
			?>
				...
				<div class="featured-item-container col-lg-3 col-md-3 col-sm-6 col-xs-12">
					...
					<div class="featured-item">
						...
						if( has_post_thumbnail( $shop_id ) ): ?>
							...
							<div class="logotype">
								...
								<div class="logotype-image">
									...
									<a href="<?php echo esc_url( get_permalink( $shop_id ) ); ?>"><?php echo get_the_post_thumbnail( $shop_id, 'medium' , array('class' => 'img-responsive' ) ); ?></a>
								...
								?>
									...
									<a href="<?php echo esc_url( get_permalink( $shop_id ) ); ?>"><img class="hover_logo" src="<?php echo esc_url( $img_data['src'] ); ?>"></a>
						...
						<div class="featured-item-content">
							...
							<a class="shop" href="<?php echo esc_url( get_permalink( $shop_id ) ); ?>"><h4><?php echo get_the_title( $shop_id ); ?></h4></a>
						...
						<div class="note view-all item-meta">
						...
						<div class="meal" style="min-height:45px;">
						 ...
						 <?php if(coupon_get_option( 'display_meals' ) > 0){?>
							...
							<ul class="list-inline list-unstyled">
								...
								<li style="width:49%;">
									...
									<a href="#_" class="info pop-over" style="width:125%;position:relative;z-index:9999;display:block;" data-title="<b>Meals</b>" data-content="<p>The <?php echo get_the_title( $shop_id ); ?> is helping feed the needy anytime you use one of their coupons. Meals to be provided <?php echo (allMealsofShop( $shop_id ))?allMealsofShop( $shop_id ):'no'; ?>.</p>"><span class="fa fa-meal"></span> <?php echo (allMealsofShop( $shop_id ))?allMealsofShop( $shop_id ):0; ?> &nbsp; meals available</a>	
								...
								<li class="">	
									...
									<a href="<?php echo get_permalink( $code->ID ) ?>">
										...
										<span class="fa fa-coupon"></span><?php echo $label; ?>
...
function coupon_categories_list( $categories ){
	...
	if( !empty( $categories ) ){
		...
		foreach( $categories as $category ){
			...
			$category_list .= '<a class="gcil" href="'.esc_url( get_category_link( $category->term_id ) ).'">'.$category->name.'</a> ';
	...
	if( !empty( $page_links ) ){
		...
		foreach( $page_links as $page_link ){
			...
			if(1 == $paged && $done ==0){
					...
					$list .='<li><a class="prev page-numbers" href="javascript:void(0);"></a></li>';
			...
			if( strpos( $page_link, 'page-numbers current' ) !== false ){
				...
				$page_link = str_replace( "<span class='page-numbers current'>", '<a href="javascript:void(0);">', $page_link );
			...
			if( count($page_links) == $count && 1 != $paged)
...

			{
				...
				$list .='<li><a class="next page-numbers" href="javascript:void(0);"></a></li>';
...
function coupon_ajax_search(){
	...
	if( !empty( $results ) ){
		...
		foreach( $results as $result ){
			...
			$shop_metad = get_post_meta(  $result->ID );

			$shop_logo_icond = coupon_get_smeta( 'shop_logo_icon',$shop_metad );

			$img_data = coupon_get_attachment( $shop_logo_icond, 'full' );

			

			$vvvr  = (coupon_get_smeta( 'shop_avg_save',$shop_metad ))?coupon_get_smeta( 'shop_avg_save',$shop_metad ):'0';

			

			if(empty($img_data['src'])){$img_data['src'] = 'http://beatific.co/wp-content/uploads/2015/09/hollister.png';}
			...
			$response["list"][] = array(
				...
				"image"	=> urlencode( $img_data['src'] ),

				

				"avg_svg"	=> urlencode( $vvvr )
...
function coupon_get_code(){
	...
	if( !empty( $code ) ){
		...
		$remainingtxt = ($remaining == 0 )?  'Expired' : $remaining;
		...
		$ratings =	( in_array( 'code', $has_ratings ) )?coupon_get_ratings($codeid):'';



		$shoplink = esc_url( coupon_get_label_link( $shop_id, $codeid ) );
		...
		$code_api = trim($code_api, '/');



		if (!preg_match('#^http(s)?://#', $code_api)) {



			$code_api = 'http://' . $code_api;
		...
		$response['code_websiteurl'] = $code_api;



		$response['code_domain'] = $domain;



		$response['code_discount'] = $code->code_discount;



		$response['code_text'] = $code->code_text;



		$response['expiry'] = $remainingtxt;
		...
		$response['coupon_label'] = $coupon_label;
		...
		$response['code_meals'] = $coupon_meals;
	...
	<div class="media">
		...
		<div class="comment-inner">
			...
			<div class="media-body comment-body">
				...
				<h4 class="media-heading"><?php comment_author(); ?><span class="fa fa-tilted-sep" style="margin-left: 20px;"></span></h4>
...
{    
	...
	if( $wp_query->is_search && $post_type == 'shop' )   
